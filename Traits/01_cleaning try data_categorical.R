################################################################################
##  cleaning try data_categorical.R: Gathering categorical trait data from TRY Plant Database
...
###NOTE: Code was originally done for Nov2019 TRY pull. In Feb 2021, we added more species and modified the code. When we changed the code (generally the second pull was much lower and some trait values were not found), the lines from the original code that no longer worked were pasted below the updated code.  This data was only pulled for CoRRE species, and does not include anything for GEx. 
...
#### Categorical trait data from our literature review ####
...
categoricalTraits <- rbind(categoricalTraitsCoRRE, categoricalTraitsGEx) %>% 
  ...
  select(-random, -spotcheck_assignment, -original_assignment, -pollination, -pollination_source, -dispersal_mode, -dispersal_mode_source, -Double.Checked.By) %>%
...
TRYcategorical <- read.csv('CleanedData/Traits/complete categorical traits/TRY_Categorical_Traits_Lookup_Table_2012_03_17_TestRelease.csv') %>% 
  ...
  select(species_matched,
         leaf_type_match, LeafType, leaf_type, leaf_type_source, leaf_type_error,
         leaf_compoundness_match, LeafCompoundness, leaf_compoundness, leaf_compoundness_source, leaf_compondness_error,
         growth_form_match, PlantGrowthForm, Succulent, Woodiness, growth_form, growth_form_source, growth_form_error,
         photosynthetic_pathway_match, PhotosyntheticPathway, Parasitic, photosynthetic_pathway, photosynthetic_pathway_source, photosynthetic_pathway_error,
         lifespan, lifespan_source, lifespan_error,
         stem_support_match, climber, Epiphyte, stem_support, stem_support_source, stem_support_error,
         clonal, clonal_source, clonal_error)
...
#   unique()
# 
# # #### 2020 Update: Subset out only new species to add on to existing spreadsheet ####
# # new <- read.csv("C:\\Users\\kjkomatsu\\Dropbox (Smithsonian)\\working groups\\CoRRE\\CoRRE_database\\Data\\CompiledData\\Species_lists\\newsp2021.csv")
# # 
# # keyNew <- key %>%
# #   right_join(new)
# 
...
# 
# # How many traits does each species have? ####
...
# 
# 
# #### Cleaning TRY categorical traits ####
# 
# # Lifespan - Trait 59
# trait59 <- dat3 %>%
#   filter(TraitID==59 & OrigValueStr!="") %>%
#   mutate(CleanTraitValue=ifelse(OrigValueStr %in% c("1", "always annual", "ann", "annual", "Annual", "annual-winter annual", "annuals", "winter annual", "winter annuals") | OriglName=="Plant phenology: Annual" & OrigValueStr=="yes" | OriglName=="Plant phenology: Perennial" & OrigValueStr=="no" | OrigValueStr=="summer annuals", "Annual", 
#         ifelse(OrigValueStr %in% c("2", "1, 2", "1,2", "1-2", "always annual, always biennial", "always biennial", "annual-winter annual, biennial", "annual, Biennial", "Annual, Biennial", "annual/bieenial", "annual/biennial", 'annual/bisannual', "biannual", "biasannual", "biennial", "sometimes annual, always biennial", "winter annual-biennial", "always annual, always biennial, always pluriennial-hapaxanthic", "strict monocarpic bi-annuals and poly-annuals") | OriglName=="Plant phenology: Biennial" & OrigValueStr=="yes", "Biennial",
...
#   mutate(CleanTraitValue=ifelse(is.na(Biennial) & is.na(Perennial), "annual",
#                          ifelse(is.na(Annual) & is.na(Perennial) | Annual=="Annual" & Biennial=="Biennial" & is.na(Perennial), "biennial", "perennial"))) %>%
#   select(species_matched, CleanTraitValue) %>%
#   mutate(CleanTraitName="lifespan", CleanTraitUnit=NA, source='TRY_59')
# 
# table(trait59$CleanTraitValue)
# 
# # Growth Form - Trait 59
# trait42 <- dat3 %>%
...
# 
# 
# # Photosynthetic pathway - Trait 22
# trait22 <- dat3 %>%
#   filter(TraitID==22) %>%
...
# 
# # code for when there were climbing species
...
# 
# 
# # Clonal growth - Traits 208, 341, 329, 334, 344, 357, 609, 613
# 
...
# 
# 
# # #43 leaf type  -- so many of these are wrong, we need to fill them by hand
# # trait43 <- dat3%>%
# #   filter(TraitID==43) %>%
# #   filter(!is.na(OrigValueStr)) %>%
# #   rename(CleanTraitValue=OrigValueStr) %>%
