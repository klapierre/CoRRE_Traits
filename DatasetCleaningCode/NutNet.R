...
cover <- read.csv('full-cover-18-October-2021.csv')%>%
  mutate(keep=ifelse(site_code %in% c('shps.us', 'yarra.au', 'cdcr.us', 'sier.us', 'veluwe.nl'), 1, ifelse(site_code %in% c('temple.us', 'bayr.de') & trt %in% c('Control', 'NPK'), 1, ifelse(site_code=='cbgb.us' & trt %in% c('Control', 'N', 'P', 'K', 'NP', 'NK', 'PK', 'NPK'), 1, 0))))%>%
  filter(keep==1)%>%
  select(-keep)
