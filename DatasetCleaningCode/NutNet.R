...
cover <- read.csv('full-cover-07-December-2020.csv')%>%
  mutate(keep=ifelse(site_code %in% c('shps.us', 'lancaster.uk', 'cbgb.us', 'lake.us', 'potrok.ar', 'yarra.au', 'cdcr.us', 'sier.us', 'chilcas.ar', 'veluwe.nl'), 1, ifelse(site_code=='temple.us' & trt %in% c('Control', 'NPK'), 1, 0)))%>%
  filter(keep==1)%>%
  select(-keep)
write.csv(cover, 'C:\\Users\\lapie\\Dropbox (Smithsonian)\\working groups\\CoRRE\\CoRRE_database\\Data\\OriginalData\\2020 update\\Data\\nutnet\\nutnet_cover_01202021.csv')
